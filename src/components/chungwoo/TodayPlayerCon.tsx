...
const TodayPlayerCon = () => {
  const { todayPlayers, setSelectedPlayerPcode, selectedPlayerPcode } = useStore((state) => ({
  ...
  const startingMember = todayPlayers.filter((changeinn) => changeinn.changeinn == "");
  const todayPlayersSub = todayPlayers.filter((changeinn) => changeinn.changeinn !== "");
  ...
  return (
    <div className="relative flex" onClick={handleParentClick}>
      <Image src="/images/DiamondField.svg" alt="야구장" width={1200} height={1200} />
      ...
      <div>
        ...
        {todayPlayersSub.map((player, index) => (
          <div onClick={handlePlayerCardClick(player.pcode)} key={index} className="my-12">
            <PlayerCard
              name={player.name}
              imageUrl={player.imageUrl}
              number={player.number}
